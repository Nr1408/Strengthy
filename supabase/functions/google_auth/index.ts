// Deployable Supabase Edge Function wrapper for google_auth
...
const SUPABASE_URL = Deno.env.get('SUPABASE_URL') || Deno.env.get('SUPABASEURL') || Deno.env.get('URL') || ''
...
const GOOGLE_CLIENT_IDS_RAW = Deno.env.get('GOOGLE_CLIENT_ID') || Deno.env.get('GOOGLE_CLIENT_IDS') || ''
const GOOGLE_CLIENT_IDS = GOOGLE_CLIENT_IDS_RAW.split(',').map(s=>s.trim()).filter(Boolean)
...
serve(async (req: Request) => {
	try {
		...
		if (GOOGLE_CLIENT_IDS.length > 0) {
			const aud = String((info as any).aud || (info as any).audience || '')
			if (!GOOGLE_CLIENT_IDS.includes(aud)) {
				return new Response(JSON.stringify({ detail: 'Google token audience mismatch' }), { status: 401 })
			}
		}
