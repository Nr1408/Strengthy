...
export default function Workouts() {
        ...
        ex.tempId === exerciseTempId
          ? {
              ...
              sets: ex.sets.map((s: any) =>
                s.id === setId ? { ...s, ...updates, _dirty: true } : s
              ),
            }
          : ex
      )
  ...
  const handleEditableSetComplete = async (
    ...
    updateEditableSet(exerciseTempId, setId, { completed: !s.completed });
      ...
      if (!isNumeric) {
        ...
        if (match) {
          ...
          const created = await createExercise(
            ex.name,
            (ex.muscleGroup as any) || "full-body",
            ex.description || ""
          );
          exId = created.id;
  ...
  function ExercisePicker({
    ...
    const muscleGroups = useMemo(() => {
      ...
      options.forEach((o) => set.add(o.muscleGroup || "other"));
    ...
    const filtered = useMemo(() => {
      ...
      return options.filter((o) => {
        if (filter !== "all" && (o.muscleGroup || "other") !== filter)
          return false;
        if (!q) return true;
        return (
