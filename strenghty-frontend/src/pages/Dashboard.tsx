...
import { getSets, getWorkouts, UiWorkoutSet, UiWorkout } from "@/lib/api";
...
export default function Dashboard() {
  ...
  const prsThisWeek = Object.values(setsByWorkoutThisWeek)
    .flat()
    .filter((s) => s.isPR).length;
  const prsLastWeek = Object.values(setsByWorkoutLastWeek)
    .flat()
    .filter((s) => s.isPR).length;
  ...
  const setsThisWeek = Object.values(setsByWorkoutThisWeek).reduce(
    (acc, sets) => acc + sets.length,
    0
  );
  const setsLastWeek = Object.values(setsByWorkoutLastWeek).reduce(
    (acc, sets) => acc + sets.length,
    0
  );
